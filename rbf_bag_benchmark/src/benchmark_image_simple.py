...
class Benchmark:
    ...
    def __run_benchmark(name, results, index):
        popen = subprocess.Popen(["rosrun", "rbf_bag_benchmark", name], shell=False, stdout=subprocess.PIPE)
        times = []
    ...
    def start(self):
        ...
        for i, benchmark in enumerate(self.benchmarks):
            t = Thread(target=self.__run_benchmark, args=("benchmark_image_simple_" + benchmark, self.results, i))
...
if __name__ == "__main__":
    ...
    subprocess.call(("rosbag", "play", bag_path, "-r 10", "-d 2"), env=os.environ.copy())
    result = b.wait()
    # print(result)
